...
main() {
    ...
    for today_date in "${today_dates[@]}"; do
        ...
        twodays_before=$(get_working_day_before "$(get_working_day_before "$full_date" "${holidays[@]}")" "${holidays[@]}")
        twodays_before_day=$(date -d "$twodays_before" +%d)
        twodays_before_month=$(date -d "$twodays_before" +%m)

        # Add twodays\.sh entry
        crontab_entries+=("0 0 $twodays_before_day $twodays_before_month * /home/id2/\.local/bin/twodays\.sh > /dev/null 2>&1")
